<template>
    ...
    <div class=" w-full lg:h-full flex flex-col">
        <div class="w-full h-[40px] justify-center text-center justify-items-center">
            <h2 class="winky text-[25px] font-bold">Incomes</h2>
        </div>
        <div
            class="flex font-bold w-full items-center justify-center text-center justify-items-center h-1/11 lg:h-1/8 mb-4">
            <span class="winky w-full text-[20px]">Total Incomes: {{ globalStore.totalIncome }}XAF</span>
        </div>
        <div class=" flex flex-col lg:flex-row w-full h-full lg:overflow-hidden flex-1">
            <div class="h-full  lg:w-1/3">
                <form action="" class="h-full flex flex-col items-center" @submit.prevent="globalStore.addIncome">
                    <div
                    ...
                    <input type="text" v-model="globalStore.title"
                    maxlength="15"
                        :class="{ 'border-red-500': isFieldInvalid(globalStore.title) }"
                    ...
                    <input type="number" v-model="globalStore.amount"
                    maxlength="9"
                        :class="{ 'border-red-500': isFieldInvalid(globalStore.amount) }"
                    ...
                    <div class="relative w-full input-control">
                        ...
                        <VueDatePicker v-model="globalStore.date" placeholder="Select a date"
                            ...
                            :class="{ 'border-red-500': isFieldInvalid(globalStore.date) }"
                            onblur="if(!this.value) this.type='text'" :enable-time-picker="false" :auto-apply="true"
                            :teleport="true" />
                    </div>
                    <select v-model="globalStore.category"
                        :class="{ 'border-red-500': isFieldInvalid(globalStore.category) }"
                        ...
                        <option value="Investisments">Investisments</option>
                    ...
                    <textarea v-model="globalStore.descr"
                        :class="{ 'border-red-500': isFieldInvalid(globalStore.descr) }"
                    ...
                    <div class="w-full flex-1 flex items-center justify-center"><button @click.prevent="submitForm"
                            ...
                            <span class="ml-1 text-nowrap winky">Add Income</span>
                        </button></div>
                ...
                <div class="w-full h-[82px]  rounded-xl p-[10px] mb-1 border border-[#e8ebed] bg-white flex flex-row justify-between"
                                    ...
                                    <div class="w-full h-1/2 flex flex-row">
                                        <div class="h-full">
                                            <div class=" h-full flex justify-center items-center "><img
                                                    src="../assets/icons8-point-final-100 green.png" class="w-[30px]"
                                                    alt=""></div>
                                        </div>
                                        <div
                                            class="winky h-full flex-1 flex justify-start items-center text-center text-nowrap p-[1px]">
                                            <span class="text-nowrap">{{ income.title }}</span>
                                        </div>
                                    </div>
                                    <div class="w-full flex-1 flex flex-row">
                                        ...
                                        <div class="hidden md:visible w-[115px] md:flex flex-row">
                                            ...
                                            <div class=" h-full flex justify-center items-center"><span
                                                    class="font-semibold">{{ formatDate(income.date) }}</span></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class=" h-full flex items-center justify-end cursor-pointer">
                        <div class="flex items-center justify-center  rounded-full bg-black w-10 h-10 "
                            @click="globalStore.deleteIncome(income._id)"><img src="../assets/trash-solid (1).svg"
...
const submitForm = () => {
    ...
    if (
        ...
        // Envoi des données à l'API via Pinia
        globalStore.addIncome();
...
onMounted(() => {
    globalStore.getIncomes(); // Récupérer la liste des revenus
});
...
const getImageByCategory = (category: string) => {
    switch (category) {
        case 'Salary':
            return '/src/assets/Salary.png';  // Replace with the actual path
        case 'Freelance':
            return '/src/assets/Freelance.png';  // Replace with the actual path
        case 'Investment':
            return '/src/assets/Investment.png';
        case 'Stocks':
            return '/src/assets/Stocks.png';
        case 'Crypto':
            return '/src/assets/Crypto.png';
        case 'Bank Transfer':
            return '/src/assets/Bank Transfer.png';
        case 'Youtube':
            return '/src/assets/Youtube.png';
        default:
            return '/src/assets/other income.png';  // Default image in case no category matches
    }
};

// const imageSrc = computed(() => (income: { category: string }) => getImageByCategory(globalStore.category));
